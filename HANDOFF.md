...
- **`engine.ts`** — the engine, ported to TypeScript against `contract.ts` (runs under Node's
  ...
  - **L6** content model + session glue — `Drill`/`Session`/`GradeOutcome`, a `STARTER_DRILLS`
    set of 198 covering the FULL map M0–M6 + P0–P6 (estimate + action + category + outs + nuts + combos + mdf/bluffs, preflop & postflop,
...
- **`curriculum.ts`** — the guided "learn" path over the L6 drills (pure data + helpers; no engine
  logic). `MODULES` groups all 198 drills into 24 ordered modules (M0–M5.10 = Pillar 1, P0–P5 + T1 ICM + T2 push/fold = Pillar 2),
...
- **`engine.test.ts`** — 620 assertions, all passing. Exact/hand-checkable, not approximate:
...
## Run it
```
node engine.test.ts            # expect: 620 passed, 0 failed (Node strips types at runtime)
