...
- DONE: play-by-play hands (v1) — a **"Play" tab**: one hand, decision by decision, graded on every
  ...
  multi-street trees too).
- DONE: opening range charts (play-by-play **v2**) — module **P1.4** "Opening ranges", the "what do I
  open?" decision via a CHART lookup (not the EV tree). `handClass(combo)` → "AKs"/"77"; `expandRange`
  turns notation ("22+", "ATs+") into hand classes; `OPENING_RANGES` = 6-max RFI by seat (UTG tightest
...
- DONE: follow-the-user's-line BRANCHING — a "live" hand that walks the real `buildTree(state)` following
  ...
  the Play tab (story + decision buttons + a line-specific recap).
- DONE: seeded deterministic DEALER — endless GENERATED branching hands. `dealBranchHand(seed)` (pure,
  mulberry32 PRNG — no Math.random/Date.now) deals hero + flop + turn + river from a seeded shuffle vs a
