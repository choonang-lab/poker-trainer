...
export const MODULES: Module[] = [
  ...
  {
    id: "M5.8", track: "P1", title: "Range & nut advantage",
    preface: "You've learned your hand's equity against a range. The last step is bigger: whose WHOLE range is ahead on a board, and — separately — who holds the nuts? The side with the range advantage applies pressure; the side with the NUT advantage gets to overbet. Board texture, not any one hand, decides both.",
    concepts: [
      { term: "Range advantage", def: "whose entire range has more equity on a given board; the side that's ahead can bet more often." },
      { term: "Nut advantage", def: "who holds more of the very strongest hands (the near-locks). Only the side with it can safely overbet — its opponent can't have the top of the range." },
      { term: "Capped range", def: "a range that can't hold the strongest hands here; a capped opponent is who you overbet against." },
      { term: "Overbet", def: "a bet larger than the pot, used when you have the nut advantage against a capped range — for max value and to polarize." },
      { term: "Board texture", def: "the shape of the board — high or low, paired, connected, one-suited — which decides which range it favors." },
    ...
    objectives: ["Judge whose range is ahead from the board texture", "C-bet big on boards that favor your range, check on boards that don't", "Overbet only when you hold the nut advantage against a capped range"],
    example: "The same raiser is ~92% on A-K-5 but only ~31% on J-T-9 — and can overbet A-A-4 (holds all the nuts) but not K-Q-J (the caller does).",
    drillIds: ["m58-high-board-advantage", "m58-coordinated-board-disadvantage", "m58-paired-board-nut-advantage", "m58-low-board-thin-advantage",
      ...
      "m58-overbet-paired-ace", "m58-overbet-river-capped", "m58-no-overbet-villain-nuts", "m58-no-overbet-coordinated"],
  },
  {
...
export const EXPLAIN: Record<string, string> = {
  ...
  "m58-overbet-paired-ace": "Overbet. On A-A-4 you hold essentially all the near-locks (aces and big pairs) and the caller holds none — their range is capped. That's a nut advantage, and a nut advantage against a capped range is exactly when you bet BIGGER than the pot: they can never have the top hand, so charge them the maximum.",
  "m58-overbet-river-capped": "Overbet. About two-thirds of your range is a near-lock here and the caller's is ~0 — a polarized nut advantage on a paired river. You bet big with the value hands (and can mix in bluffs), because the caller is capped and can't punish a large bet. Overbets come from nut advantage, not just equity.",
  "m58-no-overbet-villain-nuts": "Don't overbet — the nuts are on the CALLER's side here. K-Q-J-x gives them the straights and two pairs while your overpairs are exposed and none of your range is a near-lock. Overbetting into the range that holds the nuts just donates. Check or bet small; you only overbet when the nut advantage is yours.",
  "m58-no-overbet-coordinated": "Don't overbet. On J-T-9-8 the caller's near-lock share (~50%) dwarfs yours (~17%) — they have the made straights, you have exposed overpairs. Overbetting requires the nuts to be on your side; when the board hits the caller's range this hard, slow down and control the pot instead.",
  ...
  // H1 — play-by-play hand: big blind defends 8♥7♥
  "h1-preflop": "Call. A suited connector closing the action in the big blind is a clear defend: you're getting a great price (you already have a blind in), you're suited and connected so you flop well, and folding surrenders a pot you're priced into. Suited connectors want cheap flops in position or a good price out of it — this is the good price.",
  "h1-flop": "Call. An open-ender is 8 outs — about a third of the time you complete by the river — and this is a textbook implied-odds call: when you hit, you can stack a top pair or overpair that isn't going anywhere. Folding a draw this strong to a single c-bet, priced in, leaves money on the table. Chase draws that pay you off when they come.",
  "h1-turn": "Raise. You've made the nut straight, the button is barreling a range full of sets, two pair, and overpairs that will pay you off, and the board is wet enough that you don't want to give a free card to a bigger draw or a boat-out. Flatting the nuts here is far too passive — build the pot now while they still have a hand they can't fold.",
  "h1-river": "Bet. You have the nuts and the button checked, capping their range at hands that can still call — second pairs, sets, missed draws that bluff-catch. Checking back wins nothing extra; a value bet gets called by everything that beat itself into a call. When you hold the nuts and get checked to, your only job is to size a bet they'll pay.",
  // H2 — play-by-play hand: button opens A♠5♠ and gets paid
  "h2-open": "Open. A suited ace on the button is a clear raise — it's folded to you, only the blinds are left, and you'll have position for the whole hand. A5s makes strong top pairs, flushes, and the occasional wheel, and even when it misses you can take the pot with position. This is a standard button open, exactly as the opening chart says.",
  "h2-flop": "Bet. You flopped two pair, aces and fives — a strong hand that wants to build the pot. The big blind's calling range is full of worse aces, pairs, and draws that will pay you off, so betting extracts value and charges the draws. Checking a hand this strong just lets the pot stay small and gives free cards. C-bet for value.",
  "h2-turn": "Bet again. The 9 is a blank — you still have two pair, still ahead of the pairs and draws that called the flop. A second barrel keeps building the pot while worse hands are still willing to call, and denies equity to the flush and straight draws in villain's range. Slowing down here would leave value on the table and risk a free card.",
  "h2-river": "Bet. Third barrel for value. The king is another blank, your two pair is still good against the second pairs and busted draws the big blind got here with, and they've just checked — capping their range at hands that call rather than raise. Check back and you win nothing extra; a value bet collects from every worse hand that calls. Get the last bet in.",
